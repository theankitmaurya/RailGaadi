...
export function FavouriteStations() {
  ...
  if (!favouriteStations.length && !showAdd) {
    return (
      <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/70 dark:bg-slate-900/60 p-4 sm:p-5 backdrop-blur-md">
        ...
        <div className="flex flex-wrap gap-1.5">
          {COMMON_STATIONS.slice(0, 4).map((st) => (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg border border-dashed border-slate-300 dark:border-slate-700 text-xs font-mono text-slate-600 dark:text-slate-300 hover:border-amber-400 hover:text-amber-500 transition-colors cursor-pointer"
            >
  ...
  return (
    <div className="rounded-2xl border border-slate-200/80 dark:border-slate-800/80 bg-white/70 dark:bg-slate-900/60 p-4 sm:p-5 backdrop-blur-md">
      ...
      {showAdd && (
        <form onSubmit={handleAdd} className="mb-3 p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 flex gap-2">
          <input
            type="text"
            placeholder="Code (e.g. NDLS)"
            value={newCode}
            onChange={(e) => setNewCode(e.target.value)}
            className="w-28 px-2.5 py-1.5 text-xs uppercase font-mono rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200"
            required
            maxLength={6}
          />
          <input
            ...
            className="flex-1 px-2.5 py-1.5 text-xs rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-200"
          />
          <button
            type="submit"
      ...
      <div className="flex flex-wrap gap-2">
        {favouriteStations.map((code) => (
          <div
            key={code}
            className="group inline-flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-950/80 shadow-xs hover:border-amber-400/60 transition-all duration-150"
          >
            <Building2 className="w-3.5 h-3.5 text-slate-400 group-hover:text-amber-500" />
            <Link
              href={`/planner?from=${code}`}
              className="text-xs font-bold font-mono text-slate-800 dark:text-slate-200 hover:text-indigo-600 dark:hover:text-indigo-400"
            >
              {code}
            </Link>
            <button
              ...
              className="p-1 rounded-md text-slate-400 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 cursor-pointer transition-colors"
            >
